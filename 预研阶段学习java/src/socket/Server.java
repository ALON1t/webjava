...
public class Server {
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        while (true) {
            ...
            // 我们可以通过 inputStream（接口 按字节方式进行处理  想要按照字符方式处理 用Scanner），读取 client 发送过来的应用层数据
            InputStream inputStream = socket.getInputStream();
            Scanner scanner = new Scanner(inputStream, "UTF-8");
            String line = scanner.nextLine();       // 先读取客户端发来的消息  读
            System.out.println(line);

            // 我们可以通过将数据写入 outputStream（接口 按字节方式进行处理  想要按照字符方式处理 用Writer/PrintWrite  ），经过OS内部的TCP机制，将数据发送给 client
