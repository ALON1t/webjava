 ...
 * 4.修改config.json配置，上传图片到服务器本地路径，及访问的主机ip,port,应用上下文路径
 * 5.idea运行时，需要配置tomcat,将tomcat/webapp路径下的项目部署
 */
@WebServlet("/ueditor")
public class UEditorServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //URL获取到时都是编码后的字符串，使用时，需要先解码再使用
        String path = URLDecoder.decode(url.getPath(),"UTF-8"); //解码
        //框架提供的富文本编辑器上传功能
        MyActionEnter enter = new MyActionEnter(req,path);
        String exec = enter.exec();//执行 返回String字符串
